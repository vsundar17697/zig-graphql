//! Vendored copy of Zig 0.16.0's default test runner
//! (lib/compiler/test_runner.zig) with one fix: the fuzz-mode failure path
//! called `std.debug.writeStackTrace` with the `*std.builtin.StackTrace`
//! returned by `@errorReturnTrace()`, which no longer coerces to
//! `*const std.debug.StackTrace` in 0.16 — making every `zig build test
//! --fuzz` fail to compile. The correct API for an error-return trace is
//! `std.debug.writeErrorReturnTrace` (see the `// PATCHED` line). Delete this
//! file and the `test_runner` wiring in build.zig once the upstream runner
//! compiles under `--fuzz` again.
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    p: {
                        if (@errorReturnTrace()) |trace| {
                            // PATCHED: was writeStackTrace, which no longer
                            // accepts a *std.builtin.StackTrace.
